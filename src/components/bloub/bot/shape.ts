...
/** Capsule (stade) centree sur l'origine : la forme originale des yeux du bot. */
...
/** Estrella redondeada (sparkle / 4 puntas suaves) centrada en el origen. */
export function roundedStarPath(w: number, h: number, rFactor = 0.22, pinch = 0.2): string {
  const hw = Math.max(w, 0.01) / 2
  const hh = Math.max(h, 0.01) / 2
  const r = Math.min(hw, hh) * rFactor
  const px = hw * pinch
  const py = hh * pinch
  return (
    `M${r2(-r)} ${r2(-hh + r)}` +
    `A${r2(r)} ${r2(r)} 0 0 1 ${r2(r)} ${r2(-hh + r)}` +
    `Q${r2(px)} ${r2(-py)} ${r2(hw - r)} ${r2(-r)}` +
    `A${r2(r)} ${r2(r)} 0 0 1 ${r2(hw - r)} ${r2(r)}` +
    `Q${r2(px)} ${r2(py)} ${r2(r)} ${r2(hh - r)}` +
    `A${r2(r)} ${r2(r)} 0 0 1 ${r2(-r)} ${r2(hh - r)}` +
    `Q${r2(-px)} ${r2(py)} ${r2(-hw + r)} ${r2(r)}` +
    `A${r2(r)} ${r2(r)} 0 0 1 ${r2(-hw + r)} ${r2(-r)}` +
    `Q${r2(-px)} ${r2(-py)} ${r2(-r)} ${r2(-hh + r)}Z`
  )
}
